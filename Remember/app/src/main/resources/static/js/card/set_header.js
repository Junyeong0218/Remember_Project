...
window.onload = () => {
	...
	if(search == "" || search.substring(1, search.length).split("=")[0] != "new_card") return;
	if(search.substring(1, search.length).split("=")[1] == "true") {
		newCardLogic();
	}
}
    ...
    if (event.target.className == 'mypage_button') {
        ...
        buttons[2].onclick = () => {
			if(location.href.includes("/user")) {
				location.href = "/card?new_card=true";
				return;
			}
            newCardLogic();
        }
...
function newCardLogic() {
	...
	const add_menu_modal = document.querySelector(".modal");
	if(add_menu_modal != null) {
    ...
    add_card_form_tag.querySelector('.cancel_button').onclick = () => {
		location.href = "/card";
	}
